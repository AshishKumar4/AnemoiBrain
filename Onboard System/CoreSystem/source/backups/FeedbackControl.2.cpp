...
//intentionLock->lock();
    ...
    float FeedbackController()
    {
        try
        {
			...
			float E_term = derivative * (1 - tanh(abs(double(newError) * 0.25))) * this->CONTROLLER_E;
...
public:
    LateralController_t(std::function<void(int)> actuatorSet, std::function<float(void)> actuatorGet, int rcChannel, float CONSTANT_P = 0.8, float CONSTANT_I = 1.0, float CONSTANT_D = 200) : FeedbackController_t(actuatorSet, actuatorGet, rcChannel, CONSTANT_P, CONSTANT_I, CONSTANT_D)
    {
        ...
        this->MAX_I_BOUNDARY = 1000;
        this->CONTROLLER_P = 2;
        this->CONTROLLER_I = 0.100;
        this->CONTROLLER_D = 5000;
		E_VALUE = 100000;
...
void holdPositionLoop()
{
    while (1)
    {
        if (shouldHoldPosition)
        {
			...
			{
				FeedbackControl::Distance_Actuator.CONTROLLER_E -= 10;
			}	
			else 
			{
				printf("$$$$$");
				//FeedbackControl::Distance_Actuator.CONTROLLER_I = 0.1;
			}
        }
...
void positionalLoop()
{
	...
	//FeedbackControl::Distance_Actuator.CONTROLLER_I = 0.01;
